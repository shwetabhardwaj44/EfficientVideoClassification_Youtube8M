...
"""Binary for FineTuning Student models on the YouTube-8M dataset."""
...
def build_graph(reader,
  ...
  learning_rate_student = tf.train.exponential_decay(
      base_learning_rate,
      global_step * batch_size,
      learning_rate_decay_examples,
      learning_rate_decay,
      staircase=True)  
  tf.summary.scalar('learning_rate', learning_rate_student)

  optimizer_student = optimizer_class(learning_rate_student)
    ...
    with sv.managed_session(target, config=self.config) as sess:
      ...
      try:
        ...
        while not sv.should_stop():
          ...
          ##### Train only student network #######
          _, total_loss, global_step_val, loss_label, predictions_val, labels_val = sess.run(
              [train_student_op, final_loss, global_step, student_label_loss, student_predictions, labels] )
          ...
          if self.is_master:
            ...
            logging.info(
                ...
                ("%.2f" % gap) + " student_label_loss: " + str(loss_label)+ " final_loss: " + str(total_loss),
                task_as_string(self.task))

            sv.summary_writer.add_summary(
                utils.MakeSummary("model_student/Training_Hit@1", hit_at_one),
                global_step_val)
            sv.summary_writer.add_summary(
                utils.MakeSummary("model_student/Training_Perr", perr), global_step_val)
            sv.summary_writer.add_summary(
                utils.MakeSummary("model_student/Training_GAP", gap), global_step_val)
            sv.summary_writer.add_summary(
