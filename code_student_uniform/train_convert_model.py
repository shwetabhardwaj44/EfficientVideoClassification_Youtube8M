...
"""Binary for Converting Model from Teacher-Student to Student alone in the network on the YouTube-8M dataset."""
...
def build_graph(reader,
  ...
  optimizer_student = optimizer_class(learning_rate)
  ...
  ###################################################################
  #### create_model_inference: Create Student Model to load params
  ###################################################################
  ...
  with tf.variable_scope("model_student"):
    student_state, student_result  = model.create_model_inference(model_input_student,
    ...
    print("Names of TRAINABLE_VARIABLES in student:")
    print(names_student_vars_to_train) 
    student_predictions = student_result["predictions"]
    
    if "loss" in student_result.keys():
      student_label_loss = student_result["loss"]
    else:
    ...
    if "regularization_loss" in student_result.keys():
      student_reg_loss = student_result["regularization_loss"]
    else:
      student_reg_loss = tf.constant(0.0)

    student_reg_losses = tf.losses.get_regularization_losses(scope='model_student')
    if student_reg_losses:
      student_reg_loss += tf.add_n(student_reg_losses)

    if regularization_penalty != 0:
      tf.summary.scalar("student_reg_loss", student_reg_loss)

    # Adds update_ops (e.g., moving average updates in batch normalization) as
    # a dependency to the train_op.
    update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
    if "update_ops" in student_result.keys():
      update_ops += student_result["update_ops"]
    if update_ops:
    ...
    # Incorporate the L2 weight penalties etc. in student model ::::
    final_loss = regularization_penalty * student_reg_loss + student_label_loss
    ...
    tf.add_to_collection("student_predictions",student_predictions)
...
class Trainer(object):
  ...
  def run(self, start_new_model=False):
    ...
    """
    ################################################
    ## Remove training directory manually ALWAYS ###
    ################################################
    ...
    with tf.Graph().as_default() as graph:
      ...
      with tf.device("/gpu:%d"%FLAGS.gpu):
        ...
        saver, saver_student = self.build_model()

        global_step = tf.get_collection("global_step")[0]
        student_label_loss = tf.get_collection("student_label_loss")[0]
        student_predictions = tf.get_collection("student_predictions")[0]
        labels = tf.get_collection("labels")[0]
        train_student_op = tf.get_collection("train_student_op")[0]
        final_loss = tf.get_collection("final_loss")[0]
        init_op = tf.global_variables_initializer()

    # NEEDED FOR NEW SCOPES
    
    sess = tf.Session(config=self.config, graph=graph)
    sess.run(init_op)
    latest_checkpoint = tf.train.latest_checkpoint(FLAGS.train_dir)
    saver_student.restore(sess, latest_checkpoint)
    logging.info("saver_student loaded successfully !")

    checkpoint_path = (FLAGS.train_dir[:-1]).replace('train', '')+'finetune/model.ckpt'
    saver.save(sess, checkpoint_path)
    saver.restore(sess, checkpoint_path)
    logging.info("New student-model saved and restored successfully for finetuning!")
  ...
  def build_model(self):
    ...
    with tf.device("/gpu:%d"%FLAGS.gpu):
      ...
      # Prepare the saver to restore required vars of STUDENT only!
      vars_student = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='model_student')
      names_vars_model = [v.name for v in vars_student]
      logging.info(names_vars_model)
      ...
      return default_saver, saver_student
